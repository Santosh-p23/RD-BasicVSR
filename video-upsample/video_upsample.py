...
MODEL_PATH = "/mnt/Enterprise/server1/sanjay/flowvsr/best/best_model.pth"

#give location of images
VAL_LR_PATH = '/mnt/Enterprise/server1/sanjay/cctv_test'
DEVICE = "cuda:0" if torch.cuda.is_available() else "cpu"
TEST_BATCH_SIZE = 32
NUM_WORKERS = 4
HIGH_RES = 256#change hr and lr size as per your image.
LOW_RES = HIGH_RES//4
IMG_CHANNELS = 3
...
class ValImageDataset(Dataset):
    def __init__(self, lr_path, imgs_per_clip=100):#imgs_per_clip is the total frames in your video which model process at atime
        '''
        Args:
        ...
        self.lr_folders = sorted(glob.glob(f'{self.hr_path}/*'))
...
class BasicVSRNet(nn.Module):
    ...
    def __init__(self, mid_channels=64, num_blocks=2, spynet_pretrained='/mnt/Enterprise/server1/sanjay/flowvsr/spynet.pth'):
...
#you can even change how ou save file
                for fake in fake_curr:
                    i += 1
                    j=0
                    for img in fake:
                        save_image(img, f"/mnt/Enterprise/server1/sanjay/flowvsr/images/hr_{i}_{j}.png", normalize=False)
                        j += 1 
...
#--------------------------------------------------------
test_dataset = ValImageDataset(VAL_LR_PATH)
